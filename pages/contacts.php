...
<html>
	<head>
		<title>Libreria Ad Astra - Contattaci</title>
	...
	<body>
		 	...
		 	<div class="container-fluid">
			    ...
			    <div class="collapse navbar-collapse" id="navbarSupportedContent">
					...
					<ul class="navbar-nav me-auto mb-2 mb-lg-0">
						...
						<li class="nav-item">
							<a class="nav-link active" href="contacts.php">Contattaci</a>
						</li>
					</ul>
					<!-- ===================== ===================== ===================== -->
		...
		<form name="contact" class="container-md" role="form" method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>">
			...
			<div class="form-floating mt-4">
				<input type="text" name="name" class="form-control" maxlength='50' placeholder="" required>
				<label for="name">Nome</label>
			</div>

			<div class="form-floating mt-4">
				<textarea name="message" class="form-control" style="height: 20rem;" maxlength='1000' placeholder="" required></textarea>
				<label for="message" class="control-label">Messaggio</label>
			</div>

			<div class="form-group mt-4">
				<input type="submit" name="submit" value="Invia" class="btn btn-lg btn-danger w-100">
			</div>
			...
			<?php
				...
				{
					// ===================== Retrieve client form values =====================
						// Retrieve sender email address value
						$sender_email = $_POST['email'];

						// Retrieve sender name value
						$sender_name = $_POST['name'];

						// Retrieve sender message value
						$sender_message = $_POST['message'];
					// ===================== =========================== =====================


					// ===================== Set up mail content =====================
						// Set up local recipient mail address
						$local_recipient_mail = 'contact@localhost'; 

						// Set up the mail subject
						$subject = 'Contact form from $sender_email';

						// Set up mail body
						$mail_body = "E-Mail: $sender_email
									\nName: $sender_name
									\nMessage:
									\n$sender_message";
					// ===================== =================== =====================

					// Send the mail and save the exit status
					$mail_exit_status = mail($local_recipient_mail, $subject, $mail_body);

					// If the mail has been sent successfully
					if($mail_exit_status)
						// Print a bootstrap-formatted alert
						echo '<div class="alert alert-success mt-4">Grazie! Ti contatteremo presto</div>';
					// If the mail hasn't been sent due to issues
					else
						// Print a bootstrap-formatted error alert
						echo '<div class="alert alert-danger mt-4">Mi dispiace, c\'è stato un errore nell\'invio del tuo messaggio...</div>';
				}
